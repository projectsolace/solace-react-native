...
const LatestDataChart = ({personalityData, personalityLabels, toneData, toneLabels, imageId}) => {
...
const mapStateToProps = ({recordings, admin}) => {
  const latestPersonality = recordings.latestRecordings.personality;
  const latestTone = recordings.latestRecordings.tone;

  const personalityData = sortData(latestPersonality);
  const personalityLabels = mapLabels(latestPersonality);
  const toneData = sortData(latestTone);
  const toneLabels = mapLabels(latestTone);
